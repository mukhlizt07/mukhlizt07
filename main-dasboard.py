...
st.subheader('Berikut adalah data set dari penyewaan sepeda periode 2011-2012')
st.dataframe(days)

st.subheader('Musim-musim dengan jumlah penyewaan paling banyak')
df = pd.DataFrame(terbesar, columns=['Musim', 'Jumlah Hari'])
st.table(df)
st.write(
    """
    Tabel di atas menujukkan total hari per musim dengan jumlah penyewaan diatas 50 % dari total penyewaan tertinggi
    """ )
...
df1 = pd.DataFrame(terkecil, columns=['Musim', 'Jumlah Hari'])
st.table(df1)
st.write(
    """
    Tabel di atas menujukkan total hari per musim dengan jumlah penyewaan dibawah 50 % dari total penyewaan tertinggi
    """ )
x1 = [0, 1, 2, 3]
